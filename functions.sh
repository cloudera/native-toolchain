...
function header() {
  ...
  if [ ! -z "$ARCHIVE" ]; then
    ...
    tar xf $LPACKAGE_VERSION.tar.xz --use-compress-program xzcat
    DIR=$LPACKAGE_VERSION
  elif [ -f $LPACKAGE_VERSION.xz ]; then
    tar xf $LPACKAGE_VERSION.xz --use-compress-program xzcat
    DIR=$LPACKAGE_VERSION
  elif [ -f $LPACKAGE_VERSION.src.tar.xz ]; then
    tar xf $LPACKAGE_VERSION.src.tar.xz --use-compress-program xzcat
    DIR=$LPACKAGE_VERSION.src
  elif [ -f $LPACKAGE-src-${PKG_VERSION}.tar.xz ]; then
    tar xf $LPACKAGE-src-${PKG_VERSION}.tar.xz --use-compress-program xzcat
    DIR=$LPACKAGE-src-${PKG_VERSION}
